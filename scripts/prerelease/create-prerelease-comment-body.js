...
function createPrereleaseCommentBody() {
  ...
  console.log("### createPrereleaseCommentBody packages_versions_diff", packages_versions_diff);
  let comment_body = "";
  for (const package_name in packages_versions_diff) {
    console.log("### createPrereleaseCommentBody package_name", package_name);
    const new_version = packages_versions_diff[package_name];
    console.log("### createPrereleaseCommentBody new_version", new_version);
    ...
    console.log("### createPrereleaseCommentBody +package_name comment_body", package_name, comment_body);
  }

  console.log("### createPrereleaseCommentBody before writing down the result comment_body", comment_body);
  fs.writeFileSync(process.env.GITHUB_OUTPUT, `comment_body=${comment_body}`);
}
